...
<p>However, all statistical consultants are familiar with being presented by a client with a memory stick (formerly, a floppy disc or CD-R) of data in some proprietary binary format, for example ‘an Excel spreadsheet’ or ‘an SPSS file’. Often the simplest thing to do is to use the originating application to export the data as a text file (and statistical consultants will have copies of the most common applications on their computers for that purpose). However, this is not always possible, and <a href="spreadsheet-like-data.html#Importing-from-other-statistical-systems">Importing from other statistical systems</a> discusses what facilities are available to access such files directly from R. For Excel spreadsheets, the available methods are summarized in <a href="network-interfaces.html#Reading-Excel-spreadsheets">Reading Excel spreadsheets</a>.</p>
<p>In a few cases, data have been stored in a binary form for compactness and speed of access. One application of this that we have seen several times is imaging data, which is normally stored as a stream of bytes as represented in memory, possibly preceded by a header. Such data formats are discussed in <a href="relational-databases.html#Binary-files">Binary files</a> and <a href="connections.html#Binary-connections">Binary connections</a>.</p>
<p>For much larger databases it is common to handle the data using a database management system (DBMS). There is once again the option of using the DBMS to extract a plain file, but for many such DBMSs the extraction operation can be done directly from an R package: See <a href="importing-from-other-statistical-systems.html#Relational-databases">Relational databases</a>. Importing data via network connections is discussed in <a href="connections.html#Network-interfaces">Network interfaces</a>.</p>
<hr />
